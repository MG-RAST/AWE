...
func New_CWL_object(original interface{}, cwl_version CWLVersion, injectedRequirements []Requirement, context *ParsingContext) (obj CWL_object, schemata []CWLType_Type, err error) {
	...
	case map[string]interface{}:
		...
		case "CommandLineTool":
			...
			clt, schemata, err = NewCommandLineTool(elem, cwl_version, injectedRequirements)
			if err != nil {
		...
		case "ExpressionTool":
			...
			et, err = NewExpressionTool(elem, cwl_version, nil, injectedRequirements) // TODO provide schemata
			if err != nil {
...
func NewCWL_object_array(original interface{}) (array CWL_object_array, schemata []CWLType_Type, err error) {
	...
	case []interface{}:
		...
		for _, element := range org_a {
			...
			cwl_object, schemata_new, err = New_CWL_object(element, "", nil, nil)
			if err != nil {
